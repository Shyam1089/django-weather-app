...
class InputForm(forms.Form):
    search = forms.CharField(label=_("Select City"), max_length=50, localize=True)
...
def homepage(request):
    ...
    if request.method == 'POST':
        ...
        city = form.cleaned_data.get('search',False)
        if city:
            weather_data = get_weather_data(city, request.LANGUAGE_CODE)
            weather_data.update({"wind_direction": get_wind_direction(weather_data['wind']['deg'])})
    else:
        form = InputForm()
